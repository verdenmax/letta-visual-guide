    ...
    "07-memory-tiers.html": {
        "mcq": [
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "系统每轮把一段 &lt;memory_metadata&gt; 库存清单拼进 system",
                     "en": "Each turn the system stitches a &lt;memory_metadata&gt; inventory into the prompt"},
    ...
    "12-context-compaction.html": {
        "mcq": [
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "走了“系统提示溢出”特判：系统提示本身 ≥ 窗口，再压对话也没用",
                     "en": "It took the “system-prompt overflow” special case: the system prompt itself is ≥ the window, so compacting more conversation won't help"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "原话没被删：原始 Message 仍留在 recall 里，需要时可再捞回",
                     "en": "The originals aren't deleted: the original Message rows stay in recall and can be fetched back when needed"},
    ...
    "13-agent-state-and-loop.html": {
        "mcq": [
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "letta_v1_agent —— 它就是触发 V3 的值",
                     "en": "letta_v1_agent — it's the value that triggers V3"},
        ...
        "mcq": [
            {
                ...
                "opts": [
                    {"zh": "调一次 LLM、执行工具、把结果落库",
                     "en": "One LLM call, tool execution, and persisting the result"},
    ...
    "15-heartbeat-to-no-heartbeat.html": {
        "mcq": [
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "注入对用户隐藏、role=user 的“心跳”消息续步；工具报错时即使没要心跳也自动续一圈",
                     "en": "Injects a user-hidden role=user “heartbeat” message to continue; on a tool error it auto-continues a lap even without a requested heartbeat"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "靠 _decide_continuation：“调了工具就继续，没调就停”；模型硬塞的 request_heartbeat 会被丢掉",
                     "en": "Via _decide_continuation: “called a tool → continue, didn't → stop”; any request_heartbeat the model jams in is dropped"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "消灭“模型忘设心跳→agent 静默冻住”这类 bug；老机制靠 insert_heartbeat 给本地模型兜底",
                     "en": "Eliminates the “model forgot the heartbeat → the agent freezes silent” bug-class; the old mechanism patched it with insert_heartbeat for local models"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "元老 Agent 和第二代 V2 都还在用心跳，只有第三代 V3 把它彻底删掉",
                     "en": "Both the elder Agent and the second-gen V2 still use heartbeats; only the third-gen V3 removes it entirely"},
    ...
    "16-tool-rules.html": {
        "mcq": [
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "不执行该工具：而是合成一条 [ToolConstraintError] 错误当作“工具返回”喂回模型，让它自纠",
                     "en": "It doesn't execute the tool: instead it synthesizes a [ToolConstraintError] error as the “tool return” fed back to the model for self-correction"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "只对 child 类规则 + parent 规则求交集",
                     "en": "Only the child-family rules + parent rules are intersected"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "停止原因记 requires_approval：不执行该工具，等人批准后才放行执行",
                     "en": "The stop reason is requires_approval: the tool isn't executed; it proceeds only after a human approves"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "枚举值用历史命名（如 run_first），和子类名（如 InitToolRule）并不一致",
                     "en": "The enum values use legacy names (e.g. run_first), not matching the subclass names (e.g. InitToolRule)"},
    ...
    "17-tool-as-function.html": {
        "mcq": [
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "不行——带参的 Dict[k,v] 会 raise ValueError",
                     "en": "No — a parameterized Dict[k,v] raises ValueError"},
    ...
    "19-tool-dispatch-and-mcp.html": {
        "mcq": [
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "SandboxToolExecutor。它的类型默认是 custom，不在工厂 _executor_map 里，于是兜底进沙箱",
                     "en": "SandboxToolExecutor. Its type defaults to custom, which isn't in the factory's _executor_map, so it falls back to the sandbox"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "不在沙箱。它连接外部 MCP server 执行，Letta 本地不跑任何工具代码",
                     "en": "Not in the sandbox. It connects to an external MCP server to run it; Letta runs no tool code locally"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "LettaCoreToolExecutor，在 Letta 进程内直跑，记忆工具不进沙箱",
                     "en": "LettaCoreToolExecutor, in-process inside Letta; memory tools do not go to the sandbox"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "全部兜底走 SandboxToolExecutor：没在表里的类型一律落到沙箱，即“默认安全”",
                     "en": "They all fall back to SandboxToolExecutor: any type not in the table lands in the sandbox — “secure by default”"},
    ...
    "20-tool-sandbox-security.html": {
        "mcq": [
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "因为沙箱输出不可信，pickle.loads 会执行任意代码、等于服务端 RCE；json.loads 最坏只是假数据",
                     "en": "Because the sandbox output is untrusted, pickle.loads executes arbitrary code — a server-side RCE; json.loads at worst yields fake data"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "结果通道被污染：stdout 里混着噪声甚至伪造的假结果，校验保证读到真 payload",
                     "en": "The result channel being polluted: stdout carries noise and even forged fake results, so verification ensures the real payload is read"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "它只把回程 payload 编码从 pickle 换成 JSON，消除 RCE 面；完整性帧和去程 agent_state 的 pickle 都没动",
                     "en": "It only switched the return-leg payload encoding from pickle to JSON, eliminating the RCE surface; the integrity frame and the inbound pickling of agent_state were left untouched"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "不是。那个 .j2 模板在 v0.16.8 没被任何 .py 引用，真实脚本是 tool_sandbox/base.py 用字符串拼出来的",
                     "en": "No. That .j2 template is referenced by no .py in v0.16.8; the real script is string-assembled in tool_sandbox/base.py"},
        ...
        "mcq": [
            {
                ...
                "opts": [
                    {"zh": "按 llm_config.model_endpoint_type——它是一个 ProviderType 字符串，传进 create 后用 match/case 匹配",
                     "en": "By llm_config.model_endpoint_type — a ProviderType string passed into create and matched with match/case"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "convert_response_to_chat_completion——它把这一家的原始响应翻译成统一的 ChatCompletionResponse",
                     "en": "convert_response_to_chat_completion — it translates this provider's raw response into the unified ChatCompletionResponse"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "不是。“三方法”是教学化简——LLMClientBase 实际有 8 个 @abstractmethod",
                     "en": "No. “Three methods” is a teaching simplification — LLMClientBase actually has 8 @abstractmethod"},
        ...
        "mcq": [
            {
                ...
                "opts": [
                    {"zh": "build_request_data + convert_response_to_chat_completion——前者改请求、后者把响应翻成 OpenAI 形状",
                     "en": "build_request_data + convert_response_to_chat_completion — the first edits the request, the second translates the response into the OpenAI shape"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "排在第一个、且是必填——模型必须先写完 thinking 才轮到业务参数",
                     "en": "First, and required — the model must finish thinking before any business parameter"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "排在最后——Google 把 thinking 追加在末尾，是“排第一”规则的反例",
                     "en": "Last — Google appends thinking at the end, the exception to the “first” rule"},
            ...
            },
            {
                ...
                "opts": [
                    {"zh": "_prepare_client_kwargs 把 base_url 设成 model_endpoint——同一套 OpenAI SDK 换个 URL 就接上不同的家；这些端点大多是“OpenAI 兼容”的",
                     "en": "_prepare_client_kwargs sets base_url to model_endpoint — the same OpenAI SDK at a different URL connects to a different provider; most are “OpenAI-compatible”"},
