...
<p>到这里，约束和经济学都讲清楚了。Letta 的回应分两步：<strong>先把"窗口里到底装了啥"精确量出来，再据此决定何时压缩。</strong>"量"这件事，落在一个叫 <span class="mono">ContextWindowOverview</span> 的数据结构上——它就是整个上下文窗口的一本<strong>token 账本</strong>：</p>
...
<p>有了账本，"何时动手"就只剩一个简单判断：<strong>把当前估算的 token 数，和一个阈值比一比</strong>。这个阈值不等到窗口满了才算，而是留了余量——默认在<strong>上限的 90%</strong> 就触发，避免真的撞上"prompt 太长"的报错：</p>
...
<span class="kw">def</span> <span class="fn">get_compaction_trigger_threshold</span>(llm_config, *, force_proactive=<span class="kw">False</span>):
    <span class="cm"># GPT-5 一族更激进，靠近 90% 就主动压缩；这里给出统一的上限算法</span>
...
<p>By here, both the constraint and the economics are clear. Letta's response has two moves: <strong>first measure exactly "what's in the window," then decide when to compress.</strong> The "measure" part lives in a data structure called <span class="mono">ContextWindowOverview</span> — a <strong>token ledger</strong> for the entire context window:</p>
...
<p>With the ledger in hand, "when to act" reduces to one simple comparison: <strong>compare the current estimated token count to a threshold</strong>. That threshold doesn't wait until the window is full — it leaves headroom, firing by default at <strong>90% of the ceiling</strong> to avoid actually hitting the "prompt too long" error:</p>
...
<span class="kw">def</span> <span class="fn">get_compaction_trigger_threshold</span>(llm_config, *, force_proactive=<span class="kw">False</span>):
    <span class="cm"># GPT-5 family is more proactive near 90%; this is the unified ceiling formula</span>
    <span class="kw">return</span> <span class="fn">int</span>(llm_config.context_window * SUMMARIZATION_TRIGGER_MULTIPLIER)
</pre></div>
