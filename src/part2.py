...
<div class="card spark">
  ...
  <strong>把"思维链"做成工具的一个参数——这是这一课的"啊哈"。</strong>主流做法是指望模型自带"思考通道"，或在提示里恳求它"请一步步想"；但通道<strong>不是每家都有</strong>，恳求也<strong>可被无视</strong>。Letta 换了个根本不同的支点：既然 agent 每次动手都得调用工具、而工具调用<strong>必须符合 schema</strong>，那就把"先想"写进 schema 本身——加一个 <span class="mono">thinking</span> 参数，设成<strong>必填（required）</strong>、<strong>排第一（put_inner_thoughts_first）</strong>，描述里再明令"永远第一个生成它"。于是"推理"从"<strong>但愿它会想</strong>"变成了"<strong>调用格式里必须先写想法，否则这次调用就是非法的</strong>"。更妙的是，这招<strong>跨 provider 通用</strong>：任何支持 function calling 的模型——哪怕完全没有原生思考能力——都被这套结构<strong>逼着"先推理、后行动"</strong>。一个参数，把一条"软约束"变成了"硬约束"。
</div>
