...
<p>The ones that need their own subclass are usually those "OpenAI-compatible but one breath short" — some model rejects some parameter, or demands one extra field. Only then does a subclass step in: after <span class="mono">super()</span>, supply that one missing breath, no need to start over from scratch.</p>
...
<p>Switch subclasses and the pattern is identical, only the edited field differs: <span class="mono">GroqClient</span> forces <span class="mono">tool_choice</span> to <span class="mono">required</span>, compelling the model to call a tool every step. Which line gets edited varies by provider, but the "super first, then patch in place" skeleton never changes.</p>
