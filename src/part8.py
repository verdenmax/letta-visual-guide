...
<p><strong>③ provider traces（<span class="mono">services/telemetry_manager.py::TelemetryManager</span>）</strong>：按 <span class="mono">step_id</span> 把<strong>原始 LLM 请求/响应</strong>整包存到 postgres/clickhouse/socket。这是给<strong>调试与复盘</strong>看的——想看那次到底发了什么 payload。默认是 <span class="mono">NoopTelemetryManager</span>，返回 <span class="mono">None</span>，<strong>不存</strong>。</p>
...
<div class="cols">
  ...
  <p>按 <span class="mono">step_id</span> 存原始 LLM 请求/响应。给调试复盘。<strong>默认 Noop（不存）</strong>，需显式接上。</p>
  </div>
</div>
...
<p><strong>provider traces</strong>：默认<strong>关</strong>。<span class="mono">TelemetryManager</span> 默认是 <span class="mono">NoopTelemetryManager</span>（存什么都返回 <span class="mono">None</span>）；要留原始 payload，得显式接上 postgres/clickhouse/socket 后端。</p>
<p>一句话：<strong>计费默认有、延迟看配置、原始 payload 默认无</strong>。三套粒度不同、开关不同、用途不同，排查时先想清楚自己要哪一套。</p>
</div></details>
...
<div class="card spark"><div class="tag">💡 设计亮点</div>
<p>第 14 课那个朴素的 <span class="mono">for</span> 循环，每跑一个 <span class="mono">_step</span> 就<strong>同时甩出三份互相关联的视图</strong>。</p>
<p>一条 <span class="mono">steps</span> DB 行（<span class="mono">model</span>/<span class="mono">provider</span>/per-step token/<span class="mono">stop_reason</span>——产品＋计费）、一个 OTel <span class="mono">agent_step</span> span（延迟/分布式追踪）、以及可选的一条 provider trace（原始 LLM payload）。</p>
<p>它们被 <span class="mono">Step.trace_id = get_trace_id()</span> 缝在一起——从一行产品记录能直接跳到分布式 trace，再跳到那次原始 LLM 调用。</p>
<p>更妙的是：step 行<strong>在 LLM 调用之前</strong>就以 <span class="mono">PENDING</span>/零 token 落库、事后才翻 <span class="mono">SUCCESS</span> 填真实用量（靠 <span class="mono">StepProgression</span> 状态机 ＋ <span class="mono">finally</span> 兜底），所以连崩溃/取消的迭代都留下<strong>可归因</strong>的持久记录。</p>
<p>而同一个 loop 被<strong>三种外壳</strong>一包——同步 <span class="mono">LettaResponse</span>、SSE 流、Redis 可恢复的后台 Run——只有壳不同。一个进程内的 <span class="mono">for</span> 循环，就此变成可追踪、可取消、可恢复的<strong>异步资源</strong>。</p>
</div>
...
<p><strong>想看那次到底发了什么</strong>：拿 <span class="mono">step_id</span> 去 provider trace（若接了 <span class="mono">TelemetryManager</span>）翻原始 LLM 请求/响应；想看累计用量则 <span class="mono">GET …/usage</span>（跨 step 累加）。</p>
...
<li>三套可观测、粒度与开关都不同：<span class="mono">steps</span> 行(产品/计费，默认写)、OTel span(延迟/追踪，看 <span class="mono">app.py</span> 配置，pytest no-op)、provider trace(原始 payload，默认 <span class="mono">NoopTelemetryManager</span> 不存)，靠 <span class="mono">Step.trace_id = get_trace_id()</span> 缝合。</li>
...
<p><strong>③ provider traces (<span class="mono">services/telemetry_manager.py::TelemetryManager</span>)</strong>: keyed by <span class="mono">step_id</span>, it stores the <strong>raw LLM request/response</strong> wholesale into postgres/clickhouse/socket. This is for <strong>debugging and post-mortems</strong> — when you want to see exactly what payload went out that time. The default is <span class="mono">NoopTelemetryManager</span>, which returns <span class="mono">None</span> and <strong>stores nothing</strong>.</p>
...
<div class="cols">
  ...
  <p>Stores the raw LLM request/response keyed by <span class="mono">step_id</span>. For debugging and post-mortems. <strong>Noop by default (stores nothing)</strong>, must be wired up explicitly.</p>
  </div>
</div>
...
<p><strong>provider traces</strong>: <strong>off by default</strong>. <span class="mono">TelemetryManager</span> defaults to <span class="mono">NoopTelemetryManager</span> (returns <span class="mono">None</span> for anything you store); to keep raw payloads you must explicitly wire up a postgres/clickhouse/socket backend.</p>
<p>In one line: <strong>billing on by default, latency on config, raw payload off by default</strong>. Three different granularities, switches, and purposes — when debugging, decide first which one you need.</p>
</div></details>
...
<div class="card spark"><div class="tag">💡 Design highlight</div>
<p>That humble <span class="mono">for</span> loop from Lesson 14 throws off <strong>three interlinked views at once</strong> on every <span class="mono">_step</span> it runs.</p>
<p>One <span class="mono">steps</span> DB row (<span class="mono">model</span>/<span class="mono">provider</span>/per-step tokens/<span class="mono">stop_reason</span> — product + billing), one OTel <span class="mono">agent_step</span> span (latency/distributed tracing), and an optional provider trace (the raw LLM payload).</p>
<p>They are stitched together by <span class="mono">Step.trace_id = get_trace_id()</span> — from one product row you can jump straight to the distributed trace, then to that raw LLM call.</p>
<p>Better still: the step row lands as <span class="mono">PENDING</span>/zero tokens <strong>before the LLM call</strong> and only flips to <span class="mono">SUCCESS</span> with real usage afterward (via the <span class="mono">StepProgression</span> state machine + a <span class="mono">finally</span> fallback), so even crashed/cancelled iterations leave an <strong>accountable</strong>, durable record.</p>
<p>And the same loop, wrapped in <strong>three shells</strong> — sync <span class="mono">LettaResponse</span>, the SSE stream, a Redis-resumable background Run — differs only in the shell. An in-process <span class="mono">for</span> loop thereby becomes a traceable, cancellable, resumable <strong>async resource</strong>.</p>
</div>
...
<p><strong>Want to see exactly what was sent</strong>: take the <span class="mono">step_id</span> to the provider trace (if a <span class="mono">TelemetryManager</span> is wired up) for the raw LLM request/response; for the cumulative usage, <span class="mono">GET …/usage</span> (accumulated across steps).</p>
...
<li>Three observability ledgers, each with its own granularity and switch: the <span class="mono">steps</span> row (product/billing, written by default), the OTel span (latency/tracing, per <span class="mono">app.py</span> config, no-op under pytest), the provider trace (raw payload, default <span class="mono">NoopTelemetryManager</span> stores nothing) — stitched by <span class="mono">Step.trace_id = get_trace_id()</span>.</li>
