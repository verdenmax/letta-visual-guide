...
<p>这里有个容易被忽略的关键：<strong>传入</strong>上下文的消息（用户输入、工具返回、心跳、系统提醒）<strong>不是裸文本</strong>——每条在被模型读到之前，都被 <span class="mono">letta/system.py</span> 包成一个<strong>带类型的 JSON 事件</strong>，标明"这是什么、什么时候发生的"。（agent 自己的回复则作为结构化的 <span class="mono">Message</span> 行原样存取。）</p>
...
<p>三类最常见的事件：<strong>user_message</strong>（用户说的话）、<strong>function_response</strong>（工具执行的返回，带 <span class="mono">status</span>）、<strong>heartbeat</strong>（agent 的"心跳"——多由 agent 请求续跑或工具失败触发，也可定时）。它们都带 <span class="mono">time</span>，于是"按时间检索"和"知道多久以前"都成了可能。</p>
...
<p>两个细节值得记。其一：它默认走 <strong>hybrid</strong>——委托 <span class="mono">search_messages_async</span>（<span class="mono">search_mode</span> 默认即 hybrid），字面与语义一起算，所以换了措辞也搜得到。其二：它会<strong>滤掉 tool 消息</strong>、以及"调用 conversation_search 本身"的那条 assistant 消息，避免把搜索结果层层嵌套、越转义越乱。</p>
...
<div class="card spark">
  <div class="tag">💡 设计亮点</div>
  <strong>对话从不丢失——而且它不是一堆裸文本，是一串带类型的 JSON 事件。</strong>两件事叠在一起，才是 recall 的真正威力。第一：每条消息都是一行可持久、可检索的 <span class="mono">Message</span>，写下就永远在库里；窗口里只留 <span class="mono">message_ids</span> 指着的最近一段，其余<strong>看不见但搜得到</strong>。第二：传入消息（用户输入、工具返回、心跳）进入上下文前，都被 <span class="mono">letta/system.py</span> 包成<strong>带类型的事件</strong>——<span class="mono">user_message</span> / <span class="mono">function_response</span> / <span class="mono">heartbeat</span> 各有 <span class="mono">type</span> 和 <span class="mono">time</span>。于是 agent 不是在读一段流水文字，而是在<strong>一连串结构化事件</strong>上推理：它分得清"这是用户说的""这是工具返回的""这是定时心跳"。把这两点合起来：一部<strong>永不丢失、可按 hybrid 搜回、且每条都自带语义类型</strong>的对话史——agent 既不会"忘了说过什么"，也不会"分不清谁说的"。这正是 MemGPT 把"对话历史"也当成一等记忆来管的关键：历史不是日志垃圾堆，而是<strong>可推理、可检索的结构化记忆</strong>——也正因如此，agent 才能在三周后、十几轮对话之后，依然精准引用你随口说过的一句话。
</div>
...
<p>Here's an easily-missed key point: <strong>incoming</strong> messages (user input, tool returns, heartbeats, system alerts) are <strong>not raw text</strong> — before the model reads each one, <span class="mono">letta/system.py</span> wraps it into a <strong>typed JSON event</strong> stating "what this is and when it happened." (The agent's own replies are stored and read back as structured <span class="mono">Message</span> rows, as-is.)</p>
...
<p>The three most common events: <strong>user_message</strong> (what the user said), <strong>function_response</strong> (a tool's return, with <span class="mono">status</span>), <strong>heartbeat</strong> (a "heartbeat" — usually the agent asking to continue or a tool-failure retry, sometimes a timer). All carry <span class="mono">time</span>, so "retrieve by time" and "know how long ago" both become possible.</p>
...
<p>Two details worth remembering. One: it defaults to <strong>hybrid</strong> — delegating to <span class="mono">search_messages_async</span> (whose <span class="mono">search_mode</span> defaults to hybrid), computing text and meaning together, so rephrasing still hits. Two: it <strong>filters out tool messages</strong>, plus the assistant message that "called conversation_search itself," to avoid nesting search results layer upon layer with runaway escaping.</p>
...
<div class="card spark">
  <div class="tag">💡 Design highlight</div>
  <strong>The conversation is never lost — and it isn't a pile of raw text, it's a stream of typed JSON events.</strong> Two things stacked together are recall's real power. First: every message is one durable, searchable <span class="mono">Message</span>; once written it lives in the store forever, while only the recent slice <span class="mono">message_ids</span> points to stays in the window — the rest is <strong>invisible but searchable</strong>. Second: incoming messages (user input, tool returns, heartbeats) are wrapped by <span class="mono">letta/system.py</span> into <strong>typed events</strong> before entering context — <span class="mono">user_message</span> / <span class="mono">function_response</span> / <span class="mono">heartbeat</span>, each with <span class="mono">type</span> and <span class="mono">time</span>. So the agent isn't reading a flat stream of prose, it reasons over <strong>a sequence of structured events</strong>: it can tell "this is the user," "this is a tool return," "this is a timed heartbeat." Put them together: a conversation history that is <strong>never lost, hybrid-searchable, and self-typed per message</strong> — the agent neither "forgets what was said" nor "confuses who said it." That's the key to MemGPT treating "conversation history" as first-class memory: history isn't a junk log, it's <strong>reasoning-ready, searchable structured memory</strong> — which is exactly why the agent can, three weeks and a dozen turns later, precisely cite a line you mentioned offhand.
</div>
...
<div class="card macro">
  <div class="tag">🌍 宏观理解</div>
  <strong>一句话抓住本课：当在窗 token 逼近上限的 90%，Letta 就把较旧的对话压成一段摘要、换出原话，腾出空间继续聊——这套机制把"记忆压力"当成操作系统问题来管。</strong>触发靠一条水位线：每次模型调用后顺手记下用量（<span class="mono">context_token_estimate</span>），一旦超过 <span class="mono">context_window × 0.9</span> 这个阈值（<span class="mono">get_compaction_trigger_threshold</span>），就调 <span class="mono">compact</span> 动手。动手的方式是<strong>滑窗摘要</strong>：把较旧的一段消息交给一个 summarizer，递归地总结成一条 <span class="mono">role=summary</span> 的摘要消息插回上下文，原话退到窗外但仍留在 recall 里可搜。整个过程还能（打开开关时）抛出一个<strong>可见事件</strong>，让客户端看到"我刚做了一次压缩"。唯一压不动的是<strong>核心记忆与系统提示</strong>——它们是 system 的一部分，summarizer 只清理对话、绝不动 core，所以另有一条<strong>系统提示溢出</strong>的特判路径来兜底。
</div>
...
<div class="vflow">
  ...
  <div class="step"><div class="num">3</div><div class="sc"><h4>可选：抛事件</h4><p>开关打开时，先 yield 一个 <span class="mono">compaction</span> 事件告诉客户端"要压了"。</p></div></div>
...
<p>注意第 3 步：压完之后它会<strong>再数一遍 token</strong> 验收（仍超，会先再跑一次"全量总结"兜底）。要是连这都救不回，而且一查发现"元凶是第 0 条系统消息本身"——那就<strong>不是对话太长</strong>，是核心记忆/系统提示撑爆了预算。这条岔路就是后面要讲的"系统提示溢出"。</p>
...
<div class="vflow">
  <div class="step"><div class="num">1</div><div class="sc"><h4>算一条切割线</h4><p><span class="mono">partial_evict_summarizer_percentage</span>（默认 0.30）是<strong>保留</strong>比例：留最近 ~30%，把更早的 ~70% 总结掉。</p></div></div>
...
<p>压缩的产物是一条特殊消息。在 v3 路径里，它由 <span class="mono">letta/system.py::package_summarize_message_no_counts</span> 打包成一个 <span class="mono">type=system_alert</span> 的 JSON 事件，正文写明"开头 N 条已隐藏，以下是它们的摘要"（带"第几条/共几条"计数的老版 <span class="mono">package_summarize_message</span> 只留给 legacy <span class="mono">agent.py</span>）。</p>

<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">letta/system.py · letta/services/summarizer/compact.py</span><span class="ln">摘要消息 + role 取值（简化）</span></div>
<pre><span class="cm"># v3 路径用的打包器：不带 "第几条/共几条" 计数，故名 _no_counts</span>
<span class="kw">def</span> <span class="fn">package_summarize_message_no_counts</span>(summary, timezone, compaction_stats=<span class="kw">None</span>, mode=<span class="kw">None</span>):
    <span class="cm"># 滑窗模式：从 stats 算出驱逐了几条</span>
    num_evicted = compaction_stats[<span class="st">"messages_count_before"</span>] - compaction_stats[<span class="st">"messages_count_after"</span>]
    context_message = (
        <span class="st">f"Note: {num_evicted} messages from the beginning of the conversation "</span>
        <span class="st">f"have been hidden ... due to memory constraints.\n"</span>
        <span class="st">f"The following is a summary of the previous messages:\n {summary}"</span>
    )
    <span class="kw">return</span> <span class="fn">json_dumps</span>({<span class="st">"type"</span>: <span class="st">"system_alert"</span>, <span class="st">"message"</span>: context_message, ...})

<span class="cm"># 摘要以什么 role 入库，取决于开关 use_summary_role（源自 include_compaction_messages）</span>
<span class="kw">if</span> use_summary_role:                 <span class="cm"># 开 -> 一等公民 role=summary</span>
    m = <span class="fn">Message</span>(role=MessageRole.summary, ...)
<span class="kw">else</span>:                                <span class="cm"># 默认关 -> 退化成 role=user</span>
    m = <span class="fn">Message</span>(role=MessageRole.user, ...)
</pre></div>

<p>这条摘要消息有两副面孔。<strong>对模型</strong>（始终如此）：它就摆在上下文里（紧跟系统消息之后），让模型一眼读到"前情提要"——这一半<strong>跟开关无关</strong>。<strong>对客户端</strong>（需打开开关）：agent 才额外 yield 一组结构化事件——压缩<strong>开始前</strong>抛 <span class="mono">event_type="compaction"</span> 的 <span class="mono">EventMessage</span>，<strong>完成后</strong>抛 <span class="mono">SummaryMessage</span>，UI 才显示一条"已压缩 N 条消息"。</p>

<p>还记得第 11 课的角色表吗？<span class="mono">Message.role</span> 里那个不起眼的 <span class="mono">summary</span>，正是为这一刻准备的。打开 <span class="mono">use_summary_role</span>，摘要就以 <span class="mono">role=summary</span> 一等公民的身份入库；<strong>默认关</strong>时它退化成一条 <span class="mono">role=user</span> 的 <span class="mono">system_alert</span>——内容照样在、模型照样读，只是少了那个角色标签。</p>

<div class="note warn"><span class="ni">⚠️</span><span class="nx"><strong>一个开关，决定"可见 vs 静默"。</strong><span class="mono">include_compaction_messages</span> <strong>默认 False</strong>：此时不向客户端流式抛 <span class="mono">compaction</span> 事件，摘要也以 <span class="mono">role=user</span> 的 <span class="mono">UserMessage</span> 呈现并入库。打开它，才升级成"<span class="mono">compaction</span> 事件 + <span class="mono">role=summary</span> 一等公民"。但<strong>不管开关与否，摘要本身始终生成、始终被模型读到</strong>——可见性是<strong>能力</strong>，不是默认。</span></div>
...
<div class="card spark">
  <div class="tag">💡 设计亮点</div>
  <strong>"记忆压力"被当成一个操作系统问题来解——这正是 MemGPT 论文"队列管理器"的落地。</strong>操作系统怎么应对内存不够？它不会崩，而是<strong>换页</strong>：把不常用的页换出到磁盘，腾出物理内存。Letta 对上下文窗口做的是同一件事：到 ~90% 水位，就把较旧的对话<strong>递归地</strong>总结成一段摘要（旧摘要 + 新消息再压一层，越久越浓缩），把原话"换出"到窗外的 recall。三个设计选择让它格外像 OS：其一，<strong>有水位线</strong>——不等真满（撞 OOM）才动手，留 10% 余量主动触发；其二，<strong>换页可观测</strong>——压缩<strong>能</strong>抛一个 <span class="mono">compaction</span> 事件、把摘要以 <span class="mono">role=summary</span> 一等公民入库（由 <span class="mono">include_compaction_messages</span> 开关控制，默认关时降为静默 + <span class="mono">role=user</span>，但摘要照样生成、照样被读到），让"记忆操作"<strong>可以</strong>对 agent 与用户透明，而不是黑箱丢数据；其三，<strong>有一块"锁定内存"不可换出</strong>——核心记忆与系统提示是 agent 的身份，summarizer 永不触碰，于是必须另设一条<strong>系统提示溢出</strong>特判去处理"锁定区自己就爆了"的极端情况。把这三点合起来，你看到的不是一个"聊久了就失忆"的聊天机器人，而是一个像内核一样<strong>主动管理自身内存压力</strong>的系统——这也是 MemGPT 那篇论文最核心的隐喻：给 LLM 配一个虚拟内存管理器，让有限的上下文窗口，撑起近乎无限的对话。
</div>
...
<p><strong>源码在哪：</strong><span class="mono">letta_agent_v3.py::_create_compaction_event_message</span> 产出 <span class="mono">event_type="compaction"</span> 的 <span class="mono">EventMessage</span>（压缩<strong>开始前</strong>抛）；<span class="mono">_create_summary_result_message</span> 产出 <span class="mono">SummaryMessage</span>（压缩<strong>完成后</strong>抛）。<strong>两者都受 <span class="mono">include_compaction_messages</span> 控制</strong>（默认 False → 都不流式抛、摘要降级为 <span class="mono">UserMessage</span>）。</p>
...
<h2>收尾：记忆系统至此闭环</h2>
<p>这一课你拿下了让整个记忆系统"转得动"的那道阀门：到 90% 阈值就 <span class="mono">compact</span>，用滑窗递归摘要把较旧对话压成一条 <span class="mono">role=summary</span> 消息，过程可观测，唯独核心记忆压不动、另有溢出特判。</p>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>摘要消息</strong>：v3 路径用 <span class="mono">system.py::package_summarize_message_no_counts</span> 打成 <span class="mono">system_alert</span>；开 <span class="mono">include_compaction_messages</span> 才以 <span class="mono">role=summary</span> 入库并抛 <span class="mono">compaction</span> 事件 / <span class="mono">SummaryMessage</span>（默认关 → <span class="mono">role=user</span>、静默）。</li>
...
<div class="vflow">
  ...
  <div class="step"><div class="num">3</div><div class="sc"><h4>optional: emit event</h4><p>when enabled, first yield a <span class="mono">compaction</span> event telling the client "about to compact."</p></div></div>
...
<div class="vflow">
  <div class="step"><div class="num">1</div><div class="sc"><h4>compute a cut line</h4><p><span class="mono">partial_evict_summarizer_percentage</span> (default 0.30) is the <strong>kept</strong> ratio: keep the most recent ~30%, summarize the older ~70%.</p></div></div>
...
<p>The product of compaction is a special message. On the v3 path, <span class="mono">letta/system.py::package_summarize_message_no_counts</span> packs it into a <span class="mono">type=system_alert</span> JSON event whose body says "N messages from the start were hidden; here's their summary" (the with-counts <span class="mono">package_summarize_message</span> is reserved for the legacy <span class="mono">agent.py</span>).</p>

<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">letta/system.py · letta/services/summarizer/compact.py</span><span class="ln">summary message + role choice (simplified)</span></div>
<pre><span class="cm"># the v3 packer: drops the "of N" counts, hence _no_counts</span>
<span class="kw">def</span> <span class="fn">package_summarize_message_no_counts</span>(summary, timezone, compaction_stats=<span class="kw">None</span>, mode=<span class="kw">None</span>):
    <span class="cm"># sliding-window: derive how many were evicted from stats</span>
    num_evicted = compaction_stats[<span class="st">"messages_count_before"</span>] - compaction_stats[<span class="st">"messages_count_after"</span>]
    context_message = (
        <span class="st">f"Note: {num_evicted} messages from the beginning of the conversation "</span>
        <span class="st">f"have been hidden ... due to memory constraints.\n"</span>
...
<span class="cm"># which role the summary is stored under depends on use_summary_role (from include_compaction_messages)</span>
<span class="kw">if</span> use_summary_role:                 <span class="cm"># on -> first-class role=summary</span>
    m = <span class="fn">Message</span>(role=MessageRole.summary, ...)
<span class="kw">else</span>:                                <span class="cm"># default off -> falls back to role=user</span>
    m = <span class="fn">Message</span>(role=MessageRole.user, ...)
</pre></div>

<p>This summary message has two faces. <strong>To the model</strong> (always): it sits right in context (just after the system message), letting the model read the "story so far" at a glance — this half is <strong>switch-independent</strong>. <strong>To the client</strong> (needs the switch): only then does the agent also yield structured events — <strong>before</strong> compaction an <span class="mono">EventMessage</span> with <span class="mono">event_type="compaction"</span>, and <strong>after</strong> a <span class="mono">SummaryMessage</span> — so the UI can show "compacted N messages."</p>

<p>Remember Lesson 11's role table? That unassuming <span class="mono">summary</span> in <span class="mono">Message.role</span> was made for this moment. Turn on <span class="mono">use_summary_role</span> and the summary is stored as a first-class <span class="mono">role=summary</span> citizen; <strong>with the default off</strong> it falls back to a <span class="mono">role=user</span> <span class="mono">system_alert</span> — the content is still there and the model still reads it, just without that role label.</p>

<div class="note warn"><span class="ni">⚠️</span><span class="nx"><strong>One switch decides "visible vs silent."</strong> <span class="mono">include_compaction_messages</span> <strong>defaults to False</strong>: no <span class="mono">compaction</span> events stream to the client, and the summary surfaces and persists as a <span class="mono">role=user</span> <span class="mono">UserMessage</span>. Turn it on to upgrade to "<span class="mono">compaction</span> event + first-class <span class="mono">role=summary</span>." But <strong>switch or not, the summary itself is always created and always read by the model</strong> — visibility is a <strong>capability</strong>, not the default.</span></div>
...
<div class="card spark">
  <div class="tag">💡 Design highlight</div>
  <strong>"Memory pressure" is solved as an operating-system problem — this is exactly the MemGPT paper's "queue manager," realized.</strong> How does an OS handle low memory? It doesn't crash — it <strong>pages</strong>: swap rarely-used pages out to disk to free physical RAM. Letta does the same to the context window: at the ~90% watermark it <strong>recursively</strong> summarizes older conversation into one summary (old summary + new messages compressed one layer further, more condensed over time) and "swaps" the originals out to recall. Three design choices make it feel like an OS. First, <strong>there's a watermark</strong> — it acts at a 10% margin rather than waiting for a true OOM. Second, <strong>paging is observable</strong> — compaction <strong>can</strong> emit a <span class="mono">compaction</span> event and store the summary as a first-class <span class="mono">role=summary</span> message (gated by <span class="mono">include_compaction_messages</span>; with the default off it degrades to silent + <span class="mono">role=user</span>, yet the summary is still produced and still read), so the memory operation <strong>can</strong> be transparent to agent and user instead of a black box dropping data. Third, <strong>there's a "locked" region that can't be swapped</strong> — core memory and the system prompt are the agent's identity, the summarizer never touches them, hence a separate <strong>system-prompt overflow</strong> special case to handle "the locked region itself overflowed." Put these together and you see not a chatbot that forgets after a while, but a system that, like a kernel, <strong>actively manages its own memory pressure</strong> — the core metaphor of the MemGPT paper: give the LLM a virtual memory manager so a finite context window can sustain a near-infinite conversation.
</div>
...
<p><strong>Where in source:</strong> <span class="mono">letta_agent_v3.py::_create_compaction_event_message</span> yields an <span class="mono">EventMessage</span> with <span class="mono">event_type="compaction"</span> (<strong>before</strong> compaction); <span class="mono">_create_summary_result_message</span> yields a <span class="mono">SummaryMessage</span> (<strong>after</strong>). <strong>Both are gated by <span class="mono">include_compaction_messages</span></strong> (default False → neither streams, and the summary degrades to a <span class="mono">UserMessage</span>).</p>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>The summary message</strong>: the v3 path uses <span class="mono">system.py::package_summarize_message_no_counts</span> to pack a <span class="mono">system_alert</span>; turn on <span class="mono">include_compaction_messages</span> to store it under <span class="mono">role=summary</span> and stream a <span class="mono">compaction</span> event / <span class="mono">SummaryMessage</span> (default off → <span class="mono">role=user</span>, silent).</li>
