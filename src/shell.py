...
_FAVICON_SVG = (
    ...
    " font-weight='800' fill='#fff' text-anchor='middle'>L</text></svg>"
)
...
def head_meta(title, description, og_type="website"):
    ...
    return (
        ...
        f'<meta name="theme-color" content="#6d5efc">\n'
        f'<link rel="icon" type="image/svg+xml" href="{FAVICON}">\n'
        f'<meta property="og:type" content="{og_type}">\n'
        f'<meta property="og:site_name" content="Letta 图解教程">\n'
...
:root {
  ...
  --accent: #6d5efc; --accent-soft: #ecebfe; --accent-ink: #4733cf;
...
@media (prefers-color-scheme: dark) {
  :root {
    ...
    --accent: #9d8cff; --accent-soft: #211a45; --accent-ink: #c8bcff;
...
def page(filename, content, home_href="../index.html"):
    ...
    title_tag = f"{idx+1:02d} · {title_zh} / {title_en} - Letta 图解教程"
    desc = f"{part_zh}｜{title_zh} - Letta 图解教程（中英双语，配真实源码对应、折叠深挖与设计亮点）"
...
def index_page(lesson_prefix="lessons/"):
    ...
    title_tag = "Letta 图解教程 · 从零理解整个项目 / Letta Visual Guide"
