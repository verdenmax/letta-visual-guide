...
<h2>PR #3343: swap pickle for JSON</h2>
<p>The frame check was always there, but one old hole remained: early on, the return payload was <strong>pickle</strong>-encoded, and the server would <span class="mono">pickle.loads</span> it on read-back. That amounts to deserializing "the output of a sandbox that just ran arbitrary user code" — a clean-cut <strong>server-side RCE</strong>.</p>
<div class="codefile"><div class="cf-head"><span class="dot"></span><span class="path">letta/services/helpers/tool_parser_helper.py</span><span class="ln">PR #3343 · commit 1131535</span></div>
<pre><span class="cm"># old (vulnerable): server pickle.loads on the sandbox stdout -> server RCE</span>
<span class="cm">- result = pickle.loads(text)</span>
<span class="cm"># new (fixed): json.loads only, never executes code</span>
<span class="cm">+ result = json.loads(payload)</span>
</pre></div>
<div class="note warn"><span class="ni">⚠️</span><span class="nx">Mind the scope: the <span class="mono">marker+length+MD5</span> frame <strong>existed before #3343</strong>. #3343 only switched the sandbox→server <strong>payload encoding</strong> from pickle to JSON; server→sandbox still pickles <span class="mono">agent_state</span> — that is the trusted direction kept on purpose, not an oversight.</span></div>
<p>The fix's value is not in lines of code but in the <strong>mindset</strong>: the choice of serialization format is itself a security gate. Reserve pickle for "data I built, flowing to the low-trust side," and JSON for "data others built, returning to the high-trust side" — once internalized, this rule blocks similar RCEs at design time, instead of waiting to patch after the fact.</p>

<h2>Walk the return trip in your head</h2>
<p>Abstractions done, let us close with a concrete example. Suppose the user wrote a <span class="mono">get_weather(city)</span> tool, and at some step the model decides to call it with <span class="mono">city="Shanghai"</span>. From this moment on, here is what happens.</p>
<p>The server first <span class="mono">pickle.dumps</span> the current <span class="mono">agent_state</span> into a blob of bytes and, together with the <span class="mono">city</span> literal and the user's <span class="mono">get_weather</span> source, assembles them into a temporary script.</p>
<div class="note info"><span class="ni">💡</span><span class="nx">This step already shows the distinction: <span class="mono">agent_state</span> is pickle bytes, while <span class="mono">city</span> is just an inlined string literal <span class="mono">"Shanghai"</span> in the script. One script, two ways of "stuffing data in," matching two kinds of trust.</span></div>
<p>The script is dropped into the sandbox to run: there it <span class="mono">pickle.loads</span> the <span class="mono">agent_state</span>, runs <span class="mono">get_weather("Shanghai")</span>, gets the return value, then packs "the return value + the updated agent_state" into JSON, prefixes marker, length, MD5, and writes the whole bundle to stdout.</p>
<div class="note tip"><span class="ni">🧷</span><span class="nx">This is the lesson's turning point: from the next step on, all data has <strong>crossed the untrusted boundary</strong>. The sandbox just finished running code we could not trust in advance, so every byte it writes out must be treated as "a parcel handed over by an outsider."</span></div>
