...
> **M9 待办备注（跨课统一）**：lessons 17–23 的自测题存在系统性「长度泄题」——正确项普遍比干扰项更长、更具体（即便 `_shuffle` 打乱位置，长度/具体度仍能被读者用作线索）。L23 已先行收敛最冗长的正确项；M9 做一次**全量 quiz 题面均衡**：把引用/细节移入 `why`，让正确项与干扰项长度相当，必要时给 1–2 个干扰项补足等量具体度。
